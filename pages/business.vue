  ...
  <div class="container">
    ...
    <nav class="navbar">
      <div class="navbar-logo">
        <a href="#"> <span>C</span>ode<span>A</span>nd<span>C</span>reate </a>
      </div>
      <div class="nav-list">
        ...
        <div class="controls">
          ...
          <div class="buttons">
            <button id="play-pause">
              <font-awesome-icon class="icon" :icon="['far', playPauseClass]" />
            </button>
          </div>
        </div>
      </div>
...
const playPauseClass = ref<string>('circle-play');
onMounted(() => {
  const menu = document.querySelector('.menu');
  const navbar = document.querySelector('.navbar');

  const video = <HTMLVideoElement>document.querySelector('.video');
  const btn = document.querySelector('.buttons button .icon');
  const bar = document.querySelector<HTMLElement>('.video-bar');
  ...
  const playPause = () => {
    if (btn) {
      if (video.paused) {
        video.play();
        // btn.setAttribute('class', 'far fa-circle-pause')
        playPauseClass.value = 'circle-pause';
        // btn.className = 'far fa-circle-pause';
        video.style.opacity = '.7';
      } else {
        video.pause();
        // btn.setAttribute('class', 'far fa-circle-play')
        playPauseClass.value = 'circle-play';
        // btn.className = 'far fa-circle-play';
...
.section-1 {
  width: 100%;
  height: 100vh;
  background: url('assets/img/business/dark-bg.jpeg') no-repeat;
